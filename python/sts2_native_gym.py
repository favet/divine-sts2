...
for candidate in [REPO_ROOT / "divine-sts2" / "python", REPO_ROOT / "native_sim" / "python"]:
    if candidate.is_dir() and str(candidate) not in sys.path:
        sys.path.insert(0, str(candidate))

from sts2_native_sim import NativeWorkerPool, NativeWorker, extract_agent_observation
...
class Sts2NativeVectorEnv:
    ...
    def __init__(
        ...
        self.default_scenario = default_scenario or DEFAULT_SCENARIO
        self._rng = np.random.default_rng(seed)
        self.pool = NativeWorkerPool(num_workers)
        self.current_states: List[Dict[str, Any]] = [{}] * num_workers
        self.previous_hp: List[int] = [80] * num_workers
        self.previous_enemy_hp: List[int] = [100] * num_workers
...
if __name__ == "__main__":
    print("Testing Sts2NativeVectorEnv 5-tuple Gymnasium conformance...")
    with Sts2NativeVectorEnv(num_workers=2, seed=123) as env:
        ...
        print("  [PASS] Reset returned 2-tuple (obs, infos)")
        ...
        assert "legal_actions" in infos
        print("  [PASS] Step returned 5-tuple (obs, rewards, terminations, truncations, infos)")
        print("Sts2NativeVectorEnv Gymnasium conformance test passed!")
